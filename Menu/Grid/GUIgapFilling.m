...
GUIgapFilling_ = figure('Name','Gap Filling',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(GUIgapFilling_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(GUIgapFilling_,'Style','edit',...
...
minX_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
maxX_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
spaceX_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
nodesX_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
minY_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
maxY_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
spaceY_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
nodesY_ = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
mFator = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','4',...
    'fontUnits','normalized',...
    'TooltipString','Smoothing Factor [Use high values for data with high sampleling rate in one direction].',...
    'position',[0.3 0.525 0.32 0.08]);

iterationNumber = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','100',...
    'fontUnits','normalized',...
    'TooltipString','Iteration number [For Large datasets set few iterations].',...
    'position',[0.63 0.525 0.32 0.08]);

colorDist = uicontrol(GUIgapFilling_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
coordConversion = uicontrol(GUIgapFilling_,'Style','popupmenu',...
    'units','normalized',...
...
sz = uicontrol(GUIgapFilling_,'Style','edit',...
    'units','normalized',...
    'String','5',...
    'fontUnits','normalized',...
...
locType = uicontrol(GUIgapFilling_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
outputFile_path = uicontrol(GUIgapFilling_,'Style','edit',...
...
Cmenu = uicontextmenu(GUIgapFilling_);
set(GUIgapFilling_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
dataInterpolated = 'n';
set(GUIgapFilling_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(GUIgapFilling_);
...
guidata(GUIgapFilling_,handles);
end
...
if(dataLoaded == 'y')
    ...
    m=str2double(get(mFator,'String'));
    i=str2double(get(iterationNumber,'String'));
    
    nodesX = round(handles.nodesX);
    nodesY = round(handles.nodesY);
    x = linspace(minX,maxX,nodesX);
    y = linspace(minY,maxY,nodesY);
    [Xg,Yg] = meshgrid(x,y);
    
    MASK = histcounts2(X,Y,[x,max(x)+(x(2)-x(1))],[y,max(y)+(y(2)-y(1))]);
    MASK = MASK';
    MASK(MASK==0)=NaN;
    MASK(~isnan(MASK))=1;
    
    Zg_ = griddata(X,Y,Z,Xg,Yg,'linear');
    Zg = Zg_.*MASK;
    
    figure
    pcolor(Zg); shading interp
    
    Zg = inpaintn(Zg,i,Zg_,m);
