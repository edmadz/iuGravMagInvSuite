function GUIclassicalEnhancementFilters
...
GUIclassicalEnhancementFilters_ = figure('Name','Classical Enhancement Filters',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(GUIclassicalEnhancementFilters_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(GUIclassicalEnhancementFilters_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(GUIclassicalEnhancementFilters_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
popupFilter = uicontrol(GUIclassicalEnhancementFilters_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
    'fontUnits','normalized',...
    'String',{'ASA','THDR','TDR','TDR_THDR','TAHG','TDX','Theta-Map'},...
    'TooltipString','Enhancement filter.',...
    'position',[0.3 0.625 0.65 0.08]);

expansion_ = uicontrol(GUIclassicalEnhancementFilters_,'Style','edit',...
...
coordConversion = uicontrol(GUIclassicalEnhancementFilters_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(GUIclassicalEnhancementFilters_,'Style','pushbutton',...
    ...
    'CallBack',@applyEnhancementFilter_callBack);
...
outputFile_path = uicontrol(GUIclassicalEnhancementFilters_,'Style','edit',...
...
Cmenu = uicontextmenu(GUIclassicalEnhancementFilters_);
set(GUIclassicalEnhancementFilters_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(GUIclassicalEnhancementFilters_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(GUIclassicalEnhancementFilters_);
...
guidata(GUIclassicalEnhancementFilters_,handles);
end

%APPLY THE SELECTED ENHANCEMENT FILTER
function applyEnhancementFilter_callBack(varargin)
%Retrieve the handle structure
handles = guidata(GUIclassicalEnhancementFilters_);
...
if(dataLoaded=='y')
    ...
    if(get(popupFilter,'Value')==1)
        Zg_=sqrt(Dx.^2+Dy.^2+Dz.^2);
    elseif(get(popupFilter,'Value')==2)
        Zg_=sqrt(Dx.^2+Dy.^2);
    elseif(get(popupFilter,'Value')==3)
        Zg_=atan(Dz./sqrt(Dx.^2+Dy.^2));
    elseif(get(popupFilter,'Value')==4)
        TDR=atan(Dz./sqrt(Dx.^2+Dy.^2));
        TDR_dx=difference(Xg,Yg,TDR,'x',exp);
        TDR_dy=difference(Xg,Yg,TDR,'y',exp);
        
        Zg_=sqrt(TDR_dx.^2+TDR_dy.^2);
    elseif(get(popupFilter,'Value')==5)
        THDR=sqrt(Dx.^2+Dy.^2);
        THDR_dx=difference(Xg,Yg,THDR,'x',exp);
        THDR_dy=difference(Xg,Yg,THDR,'y',exp);
        THDR_dz=differentiate(Xg,Yg,THDR,'z',exp);
        
        Zg_=atan(THDR_dz./sqrt(THDR_dx.^2+THDR_dy.^2));
    elseif(get(popupFilter,'Value')==6)
        THDR=sqrt(Dx.^2+Dy.^2);
        Zg_=atan(THDR./abs(Dz));
    elseif(get(popupFilter,'Value')==7)
        THDR=sqrt(Dx.^2+Dy.^2);
...
if(filterApplied=='y')
    ...
    if(get(popupFilter,'Value')==1)
        flagQualitativa = 'ASA';
    elseif(get(popupFilter,'Value')==2)
        flagQualitativa = 'THDR';
    elseif(get(popupFilter,'Value')==3)
        flagQualitativa = 'TDR';
    elseif(get(popupFilter,'Value')==4)
        flagQualitativa = 'TDR_THDR';
    elseif(get(popupFilter,'Value')==5)
        flagQualitativa = 'TAHG';
    elseif(get(popupFilter,'Value')==6)
        flagQualitativa = 'TDX';
    elseif(get(popupFilter,'Value')==7)
        flagQualitativa = 'Theta_Map';
    end
