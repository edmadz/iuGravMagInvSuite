...
directionalCosine_ = figure('Name','Directional Cosine Filter',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(directionalCosine_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(directionalCosine_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(directionalCosine_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
popup = uicontrol(directionalCosine_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
    'String',{'Reject','Pass'},...
...
azimuth = uicontrol(directionalCosine_,'Style','edit',...
    'TooltipString','Structure azimuth to reject or pass [0º=Horizontal direction / 90º=Vertical direction].',...
    'units','normalized',...
    'String','0',...
    'fontUnits','normalized',...
    'Position',[0.3 0.525 0.32 0.08]);

degree = uicontrol(directionalCosine_,'Style','edit',...
    'TooltipString','Degree of cosine function.',...
    'units','normalized',...
    'String','1',...
    'fontUnits','normalized',...
    'Position',[0.63 0.525 0.32 0.08]);

expansion_ = uicontrol(directionalCosine_,'Style','edit',...
    'units','normalized',...
    'String','25',...
...
coordConversion = uicontrol(directionalCosine_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(directionalCosine_,'Style','pushbutton',...
    'units','normalized',...
    'String','Apply Direction Cosine Filter',...
    'fontUnits','normalized',...
    'position',[0.3 0.225 0.65 0.08],...
    'CallBack',@directionalCosine_callBack);
...
outputFile_path = uicontrol(directionalCosine_,'Style','edit',...
...
Cmenu = uicontextmenu(directionalCosine_);
set(directionalCosine_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(directionalCosine_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(directionalCosine_);
...
guidata(directionalCosine_,handles);
end

%APPLY DIRECTIONAL COSINE FILTER
function directionalCosine_callBack(varargin)
%Retrieve the handle structure
handles = guidata(directionalCosine_);
...
if(dataLoaded == 'y')
    ...
    theta = deg2rad(str2double(get(azimuth,'String')));
    n = str2double(get(degree,'String'));
    
    Zg_=dirCosine(Zg,type,theta,n,exp);
    ...
    if(type=='r')
        flag = ['reject_azimuth_',num2str(theta),'_degree_',num2str(n)];
    else
        flag = ['pass_azimuth_',num2str(theta),'_degree_',num2str(n)];
    end
