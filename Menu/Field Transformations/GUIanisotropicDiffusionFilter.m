function GUIanisotropicDiffusionFilter
...
GUIanisotropicDiffusionFilter_ = figure('Name','Anisotropic Diffusion Filter',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(GUIanisotropicDiffusionFilter_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(GUIanisotropicDiffusionFilter_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(GUIanisotropicDiffusionFilter_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
coordConversion = uicontrol(GUIanisotropicDiffusionFilter_,'Style','popupmenu',...
    'units','normalized',...
...
expansion_ = uicontrol(GUIanisotropicDiffusionFilter_,'Style','edit',...
...
n_ = uicontrol(GUIanisotropicDiffusionFilter_,'Style','edit',...
...
delT_ = uicontrol(GUIanisotropicDiffusionFilter_,'Style','edit',...
    'units','normalized',...
    'String','1',...
    'fontUnits','normalized',...
    'TooltipString','Time step (0 < delT <= 1).',...
    'position',[0.63 0.525 0.32 0.08]);

uicontrol(GUIanisotropicDiffusionFilter_,'Style','pushbutton',...
    'units','normalized',...
    'String','Apply Anisotropic Diffusion Filter',...
    'fontUnits','normalized',...
    'position',[0.3 0.225 0.65 0.08],...
    'CallBack',@ADF_callBack);
%--------------------------------------------------------------------------
...
outputFile_path = uicontrol(GUIanisotropicDiffusionFilter_,'Style','edit',...
...
Cmenu = uicontextmenu(GUIanisotropicDiffusionFilter_);
set(GUIanisotropicDiffusionFilter_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(GUIanisotropicDiffusionFilter_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(GUIanisotropicDiffusionFilter_);
...
guidata(GUIanisotropicDiffusionFilter_,handles);
end

%APPLY THE ANISOTROPIC DIFFUSION FILTER
function ADF_callBack(varargin)
%Retrieve the handle structure
handles = guidata(GUIanisotropicDiffusionFilter_);
...
if(dataLoaded=='y')
    ...
    delT = str2double(get(delT_,'String'));
    exp = str2double(get(expansion_,'String'));
    
    Zg_ = anisotropicDifusionFilter(Xg,Yg,Zg,exp,n,delT);
