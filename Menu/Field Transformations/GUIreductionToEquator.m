function GUIreductionToEquator
...
GUIreductionToEquator_ = figure('Name','Reduction to Magnetic Equator',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(GUIreductionToEquator_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(GUIreductionToEquator_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(GUIreductionToEquator_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
I_ = uicontrol(GUIreductionToEquator_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
D_ = uicontrol(GUIreductionToEquator_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
...
expansion_ = uicontrol(GUIreductionToEquator_,'Style','edit',...
...
coordConversion = uicontrol(GUIreductionToEquator_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(GUIreductionToEquator_,'Style','pushbutton',...
    'units','normalized',...
    'String','Apply RTE Filter',...
    'fontUnits','normalized',...
    'position',[0.3 0.225 0.65 0.08],...
    'CallBack',@rte_callBack);
...
outputFile_path = uicontrol(GUIreductionToEquator_,'Style','edit',...
...
dataLoaded = 'n';
filterApplied = 'n';
set(GUIreductionToEquator_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
%PERFORM THE REDUCTION TO THE EQUATOR FILTER
function rte_callBack(hObject,callbackdata,handles)
%Retrieve the handle structure
handles = guidata(hObject);
...
if(dataLoaded == 'y')
    ...
    rte_=reductionToEquator(Zg,I,D,exp);
    
    d = get(popupDist,'Value');
    c = get(coordConversion,'Value');
    [fig1,fig2]=generateResultFigures(650,700,Xg,Yg,Zg,rte_,d,d,'clra','clra',c);
...
function linkAxes(hObject,eventdata)
