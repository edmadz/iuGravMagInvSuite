...
hilbertTransform_ = figure('Visible','off',...
...
uicontrol(hilbertTransform_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(hilbertTransform_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(hilbertTransform_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
popupComponent = uicontrol(hilbertTransform_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
    'String',{'Hx Component','Hy Component','Total Component (eg. sqrt(Hx^2+Hy^2))'},...
    'fontUnits','normalized',...
    'position',[0.3 0.625 0.65 0.08]);

expansion_ = uicontrol(hilbertTransform_,'Style','edit',...
    'units','normalized',...
    'String','25',...
    'fontUnits','normalized',...
...
coordConversion = uicontrol(hilbertTransform_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(hilbertTransform_,'Style','pushbutton',...
    'units','normalized',...
    'String','Apply Hilbert Transform',...
    'fontUnits','normalized',...
    'position',[0.3 0.225 0.65 0.08],...
    'CallBack',@Hilbert_callBack);
...
outputFile_path = uicontrol(hilbertTransform_,'Style','edit',...
...
Cmenu = uicontextmenu(hilbertTransform_);
set(hilbertTransform_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(hilbertTransform_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(hilbertTransform_);
...
guidata(hilbertTransform_,handles);
end

%PERFORM THE HILBERT TRANSFORM
function Hilbert_callBack(varargin)
%Retrieve the handle structure
handles = guidata(hilbertTransform_);
...
if(dataLoaded=='y')
    ...
    if(get(popupComponent,'Value') == 1)
        [Zg_,~]=H_(Xg,Yg,Zg,exp,1,'on');
    elseif(get(popupComponent,'Value') == 2)
        [~,Zg_]=H_(Xg,Yg,Zg,exp,1,'on');
    elseif(get(popupComponent,'Value') == 3)
        [hx,hy]=H_(Xg,Yg,Zg,exp,1,'on');
        Zg_ = sqrt(hx.^2+hy.^2);
    end
    ...
    msgbox('Load some data before trying to apply the transformation.','Warn','warn','modal')
    return
end
