function GUIbalancedAnalyticSignal
...
GUIbalancedAnalyticSignal_ = figure('Name','Balanced Analytic Signal',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(GUIbalancedAnalyticSignal_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(GUIbalancedAnalyticSignal_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(GUIbalancedAnalyticSignal_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
k_ = uicontrol(GUIbalancedAnalyticSignal_,'Style','edit',...
    'TooltipString','Factor k is a constant that controls the effectiveness of the filter.',...
    'units','normalized',...
    'String','1',...
    'fontUnits','normalized',...
    'position',[0.3 0.625 0.65 0.08]);

expansion_ = uicontrol(GUIbalancedAnalyticSignal_,'Style','edit',...
    'units','normalized',...
    'String','25',...
    'fontUnits','normalized',...
...
coordConversion = uicontrol(GUIbalancedAnalyticSignal_,'Style','popupmenu',...
    'units','normalized',...
...
outputFile_path = uicontrol(GUIbalancedAnalyticSignal_,'Style','edit',...
...
Cmenu = uicontextmenu(GUIbalancedAnalyticSignal_);
set(GUIbalancedAnalyticSignal_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(GUIbalancedAnalyticSignal_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(GUIbalancedAnalyticSignal_);
...
guidata(GUIbalancedAnalyticSignal_,handles);
end

%APPLY THE BALANCED ANALYTIC SIGNAL
function BAS_callBack(varargin)
%Retrieve the handle structure
handles = guidata(GUIbalancedAnalyticSignal_);
...
if(dataLoaded=='y')
    ...
    [Hx,Hy]=H_(Xg,Yg,ASA,exp,1,'on');
    
    Zg_ = ASA./(k+sqrt(Hx.^2+Hy.^2+ASA.^2));
