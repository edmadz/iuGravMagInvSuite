function GUIgeneralDerivativeOperator
...
generalDerivativeOperator_ = figure('Name','Generalized Derivative Operator',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(generalDerivativeOperator_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(generalDerivativeOperator_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(generalDerivativeOperator_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
I_ = uicontrol(generalDerivativeOperator_,'Style','edit',...
    'TooltipString','Inclination of the derivative direction [degrees].',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
    'position',[0.3 0.625 0.32 0.08]);

D_ = uicontrol(generalDerivativeOperator_,'Style','edit',...
    'TooltipString','Azimuth of the derivative direction [degrees].',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
    'position',[0.63 0.625 0.32 0.08]);

expansion_ = uicontrol(generalDerivativeOperator_,'Style','edit',...
...
coordConversion = uicontrol(generalDerivativeOperator_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(generalDerivativeOperator_,'Style','pushbutton',...
    'units','normalized',...
    'String','Apply GDO Filter',...
    'fontUnits','normalized',...
    'position',[0.3 0.225 0.65 0.08],...
    'CallBack',@GDO_callBack);
...
outputFile_path = uicontrol(generalDerivativeOperator_,'Style','edit',...
...
Cmenu = uicontextmenu(generalDerivativeOperator_);
set(generalDerivativeOperator_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(generalDerivativeOperator_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(generalDerivativeOperator_);
...
guidata(generalDerivativeOperator_,handles);
end

%PERFORM THE GENERALIZED DERIVATIVE OPERATOR
function GDO_callBack(varargin)
%Retrieve the handle structure
handles = guidata(generalDerivativeOperator_);
...
if(dataLoaded == 'y')
    ...
    phi = deg2rad(str2double(get(I_,'String')));
    theta = deg2rad(str2double(get(D_,'String')));
    ...
    Zg_ = ((Dx.*sin(theta)+Dy.*cos(theta)).*cos(phi)+Dz.*sin(phi))./sqrt(Dx.^2+Dy.^2+Dz.^2);
    ...
    flag = ['Inclination_',get(I_,'String'),...
        '_Azimuth_',get(D_,'String')];
