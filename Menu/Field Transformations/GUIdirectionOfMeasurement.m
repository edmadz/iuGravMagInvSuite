function GUIdirectionOfMeasurement
...
GUIdirectionOfMeasurement_ = figure('Name','Reduction to the Pole',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(GUIdirectionOfMeasurement_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(GUIdirectionOfMeasurement_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
I_ = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
    'TooltipString','Original geomagnetic inclination (degrees).',...
    'position',[0.3 0.625 0.32 0.08]);

D_ = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
    'TooltipString','Original geomagnetic declination (degrees).',...
    'position',[0.63 0.625 0.32 0.08]);

I__ = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
    'TooltipString','New geomagnetic inclination (degrees).',...
    'position',[0.3 0.525 0.32 0.08]);

D__ = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
    'units','normalized',...
    'String','',...
    'fontUnits','normalized',...
    'TooltipString','New geomagnetic declination (degrees).',...
    'position',[0.63 0.525 0.32 0.08]);

expansion_ = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
    'units','normalized',...
    'String','25',...
    'fontUnits','normalized',...
...
coordConversion = uicontrol(GUIdirectionOfMeasurement_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(GUIdirectionOfMeasurement_,'Style','pushbutton',...
    'units','normalized',...
    'String','Apply RTP Filter',...
    'fontUnits','normalized',...
    'TooltipString','Reduce the input data to magnetic pole.',...
...
outputFile_path = uicontrol(GUIdirectionOfMeasurement_,'Style','edit',...
...
Cmenu = uicontextmenu(GUIdirectionOfMeasurement_);
set(GUIdirectionOfMeasurement_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(GUIdirectionOfMeasurement_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(GUIdirectionOfMeasurement_);
...
guidata(GUIdirectionOfMeasurement_,handles);
end

%PERFORM THE REDUCTION TO THE POLE
...
if(dataLoaded=='y')
    ...
    I_new = deg2rad(str2double(get(I__,'String')));
    D_new = deg2rad(str2double(get(D__,'String')));
    exp = str2double(get(expansion_,'String'));
    
    Zg_ = changeDirOfMeasurement(Zg,I,D,I_new,D_new,exp);
