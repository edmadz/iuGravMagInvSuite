function GUIdirectionalDerivative
...
directionalDerivative_ = figure('Name','Directional Derivative',...
    'Visible','off',...
    'NumberTitle','off',...
    'Units','pixel',...
...
uicontrol(directionalDerivative_,'Style','pushbutton',...
    'units','normalized',...
    'String','Input Data',...
...
inputFile_path = uicontrol(directionalDerivative_,'Style','edit',...
...
%--------------------------------------------------------------------------
popupDist = uicontrol(directionalDerivative_,'Style','popupmenu',...
    'units','normalized',...
    'Value',1,...
...
azimuth = uicontrol(directionalDerivative_,'Style','edit',...
    'TooltipString','Indicate the direction which derivative will be taken [0=X derivative / 90=Y derivative].',...
    'units','normalized',...
    'String','0',...
    'fontUnits','normalized',...
    'position',[0.3 0.625 0.65 0.08]);

expansion_ = uicontrol(directionalDerivative_,'Style','edit',...
...
coordConversion = uicontrol(directionalDerivative_,'Style','popupmenu',...
    'units','normalized',...
...
uicontrol(directionalDerivative_,'Style','pushbutton',...
    ...
    'CallBack',@directionalDerivative_callBack);
...
outputFile_path = uicontrol(directionalDerivative_,'Style','edit',...
...
Cmenu = uicontextmenu(directionalDerivative_);
set(directionalDerivative_,'UIContextMenu',Cmenu)
uimenu(Cmenu,'Label','Copy the GUI variables into the MATLAB workspace','Callback',@copy2MATLABworkspace);

dataLoaded = 'n';
filterApplied = 'n';
set(directionalDerivative_,'Visible','on')
%--------------------------------------------------------------------------
%CALLBACKS
%--------------------------------------------------------------------------
...
handles = guidata(directionalDerivative_);
...
guidata(directionalDerivative_,handles);
end

%PERFORM THE DIRECTIONAL DERIVATIVE
function directionalDerivative_callBack(varargin)
%Retrieve the handle structure
handles = guidata(directionalDerivative_);
